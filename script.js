...
const duplicateStatEl = document.getElementById("duplicateStat");
const missingStatEl = document.getElementById("missingStat");
const teamStatEl = document.getElementById("teamStat");
const setStatEl = document.getElementById("setStat");
...
function normaliseRarityClass(rarity) {
  return String(rarity || "").trim().toLowerCase().replace(/\s+/g, "-");
}
...
function updateStaticStats() {
  ...
  teamStatEl.textContent = teams.size;
  setStatEl.textContent = sets.size;
}
...
function renderCards(list) {
  ...
  for (const card of list) {
    ...
    const imageMarkup = card.image
      ? `<img class="card-image" src="${escapeHtml(card.image)}" alt="${escapeHtml(card.player || "Card")}" loading="lazy" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';" />
         <div class="card-image-fallback" style="display:none;">${escapeHtml(card.player || "Card")}</div>`
      : `<div class="card-image-fallback">${escapeHtml(card.player || "Card")}</div>`;

    div.innerHTML = `
      <div class="card-image-wrap">
        ${imageMarkup}
        <div class="card-id-chip">#${escapeHtml(card.id)}</div>
        <div class="card-set-chip">${escapeHtml(card.set || "")}</div>
      </div>
...
function updateStats() {
  ...
  const duplicates = Object.values(owned).reduce((sum, count) => sum + Math.max(0, count - 1), 0);
  const missing = Math.max(0, totalCards - uniqueOwnedCount);

  statsEl.textContent = `Owned ${uniqueOwnedCount} / ${totalCards} (${percent}%) • Duplicates ${duplicates}`;
  progressFillEl.style.width = `${percent}%`;
  progressPercentEl.textContent = `${percent}%`;

  duplicateStatEl.textContent = duplicates;
  missingStatEl.textContent = missing;
}
...
function renderTeamProgress() {
  ...
  for (const card of cards) {
    ...
    if (!grouped[team]) grouped[team] = { total: 0, owned: 0 };
    grouped[team].total += 1;
    if ((owned[card.id] || 0) > 0) grouped[team].owned += 1;
  }

  const teams = Object.keys(grouped).sort();
  teamSummaryEl.textContent = `Showing ${teams.length} teams`;
...
function generateTrade() {
  ...
  for (const card of cards) {
    const count = owned[card.id] || 0;
    if (count > 1) duplicates.push(`#${card.id} - ${card.player} (${count - 1} dup)`);
    if (count === 0) missing.push(`#${card.id} - ${card.player}`);
  }
...
function applySharedCollectionFromUrl() {
  ...
  const sharedState = {};
  for (const id of ids) sharedState[id] = 1;
